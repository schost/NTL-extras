...
class zz_pX_Multipoint_Geometric : public zz_pX_Multipoint{
 public:
  ...
  void evaluate(Vec<Vec<zz_p>>& val, const Vec<zz_pX>& f) const;
  void interpolate(zz_pX& f, const Vec<zz_p>& val) const;
  // TODO: write those for the general case as well
  void mul_right(Vec<zz_p>& output, const Vec<zz_p>& input) const;
  void mul_left(Vec<zz_p>& output, const Vec<zz_p>& input) const;
  void inverse_mul_right(Vec<zz_p>& output, const Vec<zz_p>& input) const;
  void inverse_mul_left(Vec<zz_p>& output, const Vec<zz_p>& input) const;
...
class zz_pX_Multipoint_CTFT : public zz_pX_Multipoint {
 public:
  ...
  /* input has length N                                        */
  /*-----------------------------------------------------------*/
  void CRT_t(long* x, long *tmp) const;
  ...
  /* input length = 2^k                                        */
  /*-----------------------------------------------------------*/
  void evaluate_chunk_t(zz_p * val, const long * coeffs, const long k, long * wk) const;

  /*-----------------------------------------------------------*/
  /* transpose-interpolates a chunk of length 2^k              */
  /* input length = 2^k                                        */
  /*-----------------------------------------------------------*/
  void interpolate_chunk_t(long * val, const zz_p * coeffs, const long k, long * wk) const;

  /*---------------------------------------------------------*/
  /* transpose-evaluates A at n points                       */
  /*---------------------------------------------------------*/
  void evaluate_t(Vec<zz_p>& val, const Vec<zz_p>& A) const;

  /*---------------------------------------------------------*/
  /* transpose-interpolates val at n points                  */
  /*---------------------------------------------------------*/
  void interpolate_t(Vec<zz_p>& f, const Vec<zz_p>& val) const;
  ...
  /*-----------------------------------------------------------*/
  /* applies the multi-reduction map to f,g (mod p), in place  */
  /* input is in [0,p), output is in [0,2p)                    */
  /* x must have size at least 2n, and upper half must be zero */
  /*-----------------------------------------------------------*/
  void bi_reduce(long* f, long *g) const;

  /*---------------------------------------------------------*/
  /* negacyclic convolution in length 2^k                    */
  /* wk and wk2 are workspaces of length 2^k                 */
  /*---------------------------------------------------------*/
  void negacyclic_convolution(long *c, const long *a, const long *b, const long k, long *wk, long *wk2) const;

  /*---------------------------------------------------------*/
  /* multiplies a and b                                      */
  /*---------------------------------------------------------*/
  void mul(zz_pX& C, const zz_pX& A, const zz_pX& B) const;
