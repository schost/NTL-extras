...
void test(long i, long j, long alpha, unsigned long rk, int opt){
  ...
  lzz_p_cauchy_like_geometric M, invM;
  Mat<zz_p> Z, invZ;
  long r;
  ...
  if (opt == 1){
    ...
    if (true) {
      ...
      M = lzz_p_cauchy_like_geometric(A, B, to_zz_p(1), power(a, i), a);
      M.to_dense(Z);
      r = invert_direct(invM, M);
      invM.to_dense(invZ);
      Mat<zz_p> Z2 = Z;
      assert (r == gauss(Z2));
      Mat<zz_p> subZ;
