...
void check(int opt){
  ...
  for (long i = 2; i < 100; i += 1){
    ...
    if (opt == 1){
      ...
      last_row_of_block(col, 0, MH);
      for (long j = 0; j < i; j++)
	assert (col[j] == h00(j, 1));
      for (long j = 0; j < 2; j++)
	assert (col[j+i] == h01(j, 1));
      for (long j = 0; j < i; j++)
	assert (col[j+i+2] == h02(j, 1));

      last_row_of_block(col, 1, MH);
      for (long j = 0; j < i; j++)
	assert (col[j] == h10(j, i-2));
      for (long j = 0; j < 2; j++)
	assert (col[j+i] == h11(j, i-2));
      for (long j = 0; j < i; j++)
	assert (col[j+i+2] == h12(j, i-2));
