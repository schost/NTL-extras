...
void check(int opt){
  ...
  for (long i = 2; i < 100; i += 1){
    ...
    if (opt == 1){
      Vec<zz_p> col;
      last_column_of_block(col, 0, MH);
      for (long j = 0; j < 2; j++)
	assert (col[j] == h00(j, i-1));
      for (long j = 0; j < i-1; j++)
	assert (col[j+2] == h10(j, i-1));

      last_column_of_block(col, 1, MH);
      for (long j = 0; j < 2; j++)
	assert (col[j] == h01(j, 2-1));
      for (long j = 0; j < i-1; j++)
	assert (col[j+2] == h11(j, 2-1));

      last_column_of_block(col, 2, MH);
      for (long j = 0; j < 2; j++)
	assert (col[j] == h02(j, i-1));
      for (long j = 0; j < i-1; j++)
	assert (col[j+2] == h12(j, i-1));
