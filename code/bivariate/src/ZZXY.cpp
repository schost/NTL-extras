...
/*------------------------------------------------------------*/
void ZZXY::eval(ZZX & val, const ZZX & g, long t){
  long d = degY();
...
/*------------------------------------------------------------*/
void ZZXY::eval(ZZX & val, ZZ & dval, const ZZX & g, const ZZ & dg, long t){
  long d = degY();
...
/*------------------------------------------------------------*/
void ZZXY::series_solution(ZZX & g, ZZ & den_g, const ZZ & g0, long t){
  clear(g);
  SetCoeff(g, 0, g0);
  den_g = 1;
  long prec = 1;
  ...
  while (prec < t){
    prec = 2*prec;
    ZZX valF, valdF, i_dF;
    ZZ dvalF, dvaldF, d_idF;
    eval(valF, dvalF, g, den_g, prec);
    dyF.eval(valdF, dvaldF, g, den_g, prec);
    inverse_series(i_dF, d_idF, valdF, dvaldF, prec);
    valF = trunc(valF * i_dF, prec);
    dvalF = dvalF * d_idF;
    g = dvalF * g - den_g * valF;
    den_g = den_g * dvalF;
    simplify(g, den_g);
  }
  g = trunc(g, t);
